...
bool StandardDialogTransition() {
    ...
    if (!OynonGetActiveInstruction(&op, &count) || count > 1000000) return false;
    char name[128]{};
    bool readable = true;
...
BOOL __stdcall Transit(const OynonCameraTransitCall* call, void*) {
    ...
    if (fraction <= 0.0f) return OynonProceedCameraTransit(call, nullptr);
    OynonCameraTransitState state{};
    if (!OynonReadCameraTransit(call, &state)) return OynonProceedCameraTransit(call, nullptr);
...
bool InstallDialogCamera() { return OynonInstallCameraTransitHook(Transit, nullptr) != FALSE; }
}
