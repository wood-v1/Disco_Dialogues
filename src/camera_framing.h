...
inline bool FrameOnLeft(Vector3& direction, const Vector3& up, float horizontalFov, float freeFraction) {
    if (!std::isfinite(horizontalFov) || horizontalFov < 0.1f || horizontalFov > 3.0f ||
        !std::isfinite(freeFraction) || freeFraction < 0.4f || freeFraction >= 1.0f) return false;
    Vector3 forward = direction;
    Vector3 right = Cross(up, forward);
    if (!Normalize(forward) || !Normalize(right)) return false;
    ...
    Vector3 framed{forward.x+right.x*tangent, forward.y+right.y*tangent, forward.z+right.z*tangent};
    if (!Normalize(framed)) return false;
    direction = framed;
    return true;
}
}
