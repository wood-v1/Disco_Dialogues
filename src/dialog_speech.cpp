...
void SpeechTrace(const char* format, ...) {
    if (!speechLog || speechLogLines++ >= 10000)
        return;
...
int __stdcall Event(const OynonScriptEventCall* call, void*) {
    ...
    ReplyScope scope{
        call->event == 11 && call->count == 2 && ShouldPreserveDialogSpeech() ? call->script : nullptr};
...
bool SpeakingInReply(const OynonSpeechCall* call, void*& actor) {
    if (!replyScope || !replyScope->script)
        return false;
    ...
    actor = state.actor;
    if (!actor || !state.loaded || !state.hasHead || state.hasWaiter)
        return false;

    void* script = nullptr;
    if (!OynonReadActorScript(call, &script) || script != replyScope->script)
        return false;

    return std::isfinite(state.current) && std::isfinite(state.end) && state.current >= 0 &&
...
BOOL __stdcall Speech(const OynonSpeechCall* call, void*) {
    ...
    if (speechLog) {
        ...
        if (OynonReadActorSpeech(call, &state))
            ...
            SpeechTrace("%s unreadable actor", name);
    }
    ...
    if (call->count == (stop ? 0u : 1u) && SpeakingInReply(call, actor)) {
        ...
        if (replyScope->preservedActor == actor)
            return TRUE;
    }
...
void __stdcall Resume(const OynonSoundResumeCall* call, void*) {
    if (speechLog)
        SpeechTrace("resume collection=%p flags=%x mask=%x enabled=%d", call->collection, call->flags,
            call->mask, ShouldPreserveDialogSpeech());
...
bool InstallDialogSpeech() {
    ...
    if (!OynonInstallScriptAudioHooks(&callbacks, nullptr))
        return false;
