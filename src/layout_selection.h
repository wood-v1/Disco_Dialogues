...
struct Layout { int width; int height; const char* xml; };
...
inline const char* SelectLayout(const char* host, int width, int height) {
    ...
    if (!host || std::strcmp(host, "dialog.xml") != 0) return nullptr;
    for (const auto& layout : layouts)
        if (layout.width == width && layout.height == height) return layout.xml;
    return nullptr;
}
}
