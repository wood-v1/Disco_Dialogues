...
static bool KeyDown(unsigned key) { return keys[key]; }
static BOOL Gate() { return modal.Blocks(KeyDown); }
static DWORD Generation() { return modal.Generation(); }
static BOOL Overlay() { return OYNON_INVENTORY_OVERLAY_NONE; }
static BOOL Apply(const char*) { ++effects; return TRUE; }
static BOOL Command(const char*) { ++commands; return TRUE; }
static BOOL RegisterConsole(OynonConsoleCommandFilter, void*) { return TRUE; }
static BOOL RegisterKeyboard(OynonKeyboardCallback, void*) { return TRUE; }
...
DWORD ReadHandCombatKey(const std::string&) { return 'Q'; }
}
...
int main() {
    ...
    for (unsigned key : std::array<unsigned, 12>{'1', '2', '3', '4', '5', 'Q', 'E', 0x61, 0x62, 0x63, 0x64, 27}) {
    ...
    modal.Open(&station); modal.Open(&unrelated);
    modal.Close(&station, KeyDown);
    Check(Gate());
    modal.Close(&unrelated, KeyDown);
    Gate();
