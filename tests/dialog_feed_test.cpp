...
bool __fastcall GetInt(Var* var, void*, int& value) {
    value = var->value;
    return var->valid;
}
...
int& Y() {
    return *reinterpret_cast<int*>(window.data() + 0xc8);
}

int& Height() {
    return *reinterpret_cast<int*>(window.data() + 0xd0);
}

bool& Clip() {
    return *reinterpret_cast<bool*>(window.data() + 0xe3);
}
...
int main() {
    ...
    expectedName = "OtherNative";
    expectedCount = 2;
    CHECK(!invoke("OtherModCommand", 4));
    CHECK(token == 1 && Y() == 20 && Height() == 100 && !Clip());

    std::puts(
        "PASS: UI forwarding, mod command validation, scoped viewport/error restoration, unrelated client callback");
}
