...
int main(int argc, char** argv) {
    ...
    if (!IsDialogCameraBlock(20, 60, lookup))
        return 1;
    if (IsDialogCameraBlock(0, 60, lookup) || IsDialogCameraBlock(20, 30, lookup))
        return 2;
    ...
    if (argc == 2) {
        ...
        while (std::getline(input, line)) {
            ...
            stream >> expected;
            for (auto& value : fixture)
                if (!(stream >> value))
                    return 7;

            bool result = IsDialogCameraBlock(
                9, 42, [&](unsigned i) { return fixture[i] == "-" ? nullptr : fixture[i].c_str(); });
