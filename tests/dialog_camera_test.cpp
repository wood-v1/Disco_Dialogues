...
struct Var {
    void** table;
    OynonVector3 direction{0, 0, 1};
    unsigned reads = 0, writes = 0;
    bool writable = true;
};

bool __fastcall Read(Var* var, void*, OynonVector3& value) {
    ++var->reads;
    value = var->direction;
    return true;
}
...
void* __fastcall Next(Object* object, void*) {
    return object->next;
}

float __fastcall Fov(void*, void*) {
    return 1.0f;
}

std::array<const char*, 60> names{};

const char* __fastcall Name(void*, void*, unsigned index) {
    return names[index];
}
...
bool __fastcall Native(void*, void*, void*& instance, void** args, unsigned count, void* result) {
    ...
    if (failNative)
        throw std::runtime_error("transit error");
    return false;
}
...
int main() {
    ...
    void* varTable[12]{};
    varTable[11] = reinterpret_cast<void*>(&Read);
    varTable[5] = reinterpret_cast<void*>(&Write);
    Var direction{varTable};

    void* cameraTable[11]{};
    cameraTable[10] = reinterpret_cast<void*>(&Fov);
    Object camera{cameraTable, nullptr};

    void* worldTable[35]{};
    worldTable[34] = reinterpret_cast<void*>(&Next);
    Object world{worldTable, &camera};

    void* contextTable[9]{};
    contextTable[8] = reinterpret_cast<void*>(&Next);
    Object context{contextTable, &world}, self{nullptr, &context};

    void* args[]{nullptr, &direction};
    expectedArgs = args;

    std::array<char, 0x38> script{};
    std::array<char, 0x30> data{};
    void* nativeTable[1]{};
    nativeInstructionTable = nativeTable;
    std::array<Op, 60> ops{};
    std::array<void*, 60> code{};
    ...
    *reinterpret_cast<unsigned*>(data.data() + 0x2c) = 20;
    ...
    names[11] = "IsOverrideActive";
    names[17] = "StopWorld";
    names[20] = "CameraTransit";
    names[24] = "Rotate";
    names[33] = "HasAnimationTrack";
    names[37] = "LookAsyncCamera";
    names[39] = "CameraWaitForPlayFinish";
    names[41] = "ResumeWorld";

    auto invoke = [&] { return Transit(&self, nullptr, expectedInstance, args, 2, args); };

    shifted = true;
    CHECK(!invoke());
    CHECK(direction.direction.x == 0 && direction.direction.z == 1 && direction.reads == 1 &&
          direction.writes == 2);
    ...
    try {
        Instruction(nullptr, nullptr, data.data(), 0);
        CHECK(false);
    } catch (const std::runtime_error&) {
    }
    ...
    std::puts(
        "PASS: camera policy, ABI forwarding, fallback, scoped vector/interpreter restoration, unrelated client callback");
}
