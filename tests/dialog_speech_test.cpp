...
void* __fastcall ControllerScript(Controller* controller, void*) {
    if (!controller->task) return nullptr;
    auto scripted = hd::Method<void*(__thiscall*)(void*, unsigned)>(controller->task, 2)(controller->task, 0x3443112b);
...
struct Actor {
    ...
    template<class T> void Set(size_t offset, T value) { std::memcpy(bytes.data() + offset, &value, sizeof(value)); }
...
struct Native { void* table = nullptr; void* actor; };
...
bool Stop(Native& native, unsigned count = 0) { return StopSpeech(&native, nullptr, instance, arguments, count, result); }
bool Play(Native& native, unsigned count = 1) { return PlaySpeech(&native, nullptr, instance, arguments, count, result); }
...
void __fastcall ResumeSound(Sound* sound, void*) {
    ...
    if (sound->paused <= 0) sound->position = 0;
    --sound->paused;
}
int __fastcall PauseState(Sound* sound, void*) { return sound->paused; }
struct Collection {
    void* table = nullptr;
    void* engine = nullptr;
...
void __fastcall NativeResume(Collection* collection, void*, unsigned flags, unsigned mask) {
    for (auto entry = collection->begin2d; entry != collection->end2d; ++entry)
        if (!((entry->flags ^ flags) & mask)) hd::Method<void(__thiscall*)(void*)>(entry->sound, 8)(entry->sound);
    for (auto entry = collection->begin3d; entry != collection->end3d; ++entry)
        if (!((entry->flags ^ flags) & mask)) hd::Method<void(__thiscall*)(void*)>(entry->sound, 9)(entry->sound);
}
void TestExitResume(bool buffered) {
    ...
    std::puts("PASS: exit resume regression; unpaused voice continues, paused audio resumes, other sounds and masks preserved");
}
...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < 10000; ++i) Event(&script1);
    ...
    for (int mode = 0; mode < 12; ++mode) {
        actor.Start(&script1);
        if (mode == 0) actor.Set<float>(0x334, 0);
        if (mode == 1) actor.Set<float>(0x330, 7);
        if (mode == 2) actor.Set<float>(0x330, -1);
        if (mode == 3) actor.Set<float>(0x334, std::numeric_limits<float>::infinity());
        if (mode == 4) actor.Set<float>(0x330, std::numeric_limits<float>::quiet_NaN());
        if (mode == 5) actor.Set<unsigned char>(0x24, 0);
        if (mode == 6) actor.Set<void*>(0x300, nullptr);
        if (mode == 7) actor.Set<void*>(0x32c, &script2); // Existing latent waiter.
        if (mode == 8) actor.Set<void*>(0x16c, nullptr);
        if (mode == 9) actor.controller.task = nullptr;
        if (mode == 10) actor.task.supportsScript = false;
        if (mode == 11) actor.task.script = &script2;
    ...
    actor.Start(&script1);
    other.Start(&script2);
    handler = [&](void*, unsigned, unsigned) { CHECK(!Stop(otherNative)); CHECK(Stop(native)); };
    Event(&script1);
    ...
    for (unsigned cleanup : {6u, 32u, 42u}) {
        ...
        handler = [&](void* script, unsigned event, unsigned) {
            if (event != 11) { CHECK(!Stop(native)); return; }
            CHECK(Stop(native));
    ...
    handler = [&](void* script, unsigned, unsigned) {
        if (script == &script2) { CHECK(Stop(otherNative)); CHECK(Play(otherNative)); return; }
        CHECK(Stop(native));
        Event(&script2);
        CHECK(Play(native));
    };
    ...
    try { Event(&script1); CHECK(false); } catch (const std::runtime_error&) {}
    CHECK(replyScope == nullptr);
    actor.Start(&script1);
    CHECK(!Stop(native));
