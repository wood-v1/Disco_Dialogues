...
int main() {
    ...
    for (const auto& size : sizes) for (float fov : fovs) {
        ...
        if (std::fabs(pixel-size[1]/2.0f) > 0.01f || direction.x <= 0) return 2;
        if (std::fabs(Dot(direction,direction)-1) > 0.00001f) return 3;
        Vector3 repeated = original;
        FrameOnLeft(repeated, up, fov, fraction);
        if (std::fabs(Dot(direction,repeated)-1) > 0.00001f) return 4;
    }
    Vector3 unchanged{0,0,1};
    if (FrameOnLeft(unchanged,{0,1,0},1,0) || FrameOnLeft(unchanged,{0,1,0},1,1) ||
        FrameOnLeft(unchanged,{0,1,0},0,0.65f) || FrameOnLeft(unchanged,{0,0,1},1,0.65f)) return 5;
    if (unchanged.x != 0 || unchanged.z != 1) return 6;
    std::puts("PASS: NPC projects to center of left region at 3 resolutions / 4 FOVs; no accumulation; invalid inputs unchanged");
    return 0;
}
